...
## 경영분쟁 참여 임원 배지 (2026-02-05) ⭐

### 기능 설명

임시주주총회 공시에서 경영분쟁 관련 임원 선임 시도 이력을 추출하여, 관계 그래프에서 해당 임원에게 경고 배지를 표시합니다.

### 표시 기준

| 조건 | 표시 | 위치 |
|------|------|------|
| 분쟁 참여 1회 이상 | 빨간색 "분쟁N" 배지 | 노드 좌측 상단 |

### UI 구현

**ForceGraph.tsx**
- 노드 좌측 상단에 빨간색 원형 배지 (`#DC2626`)
- "분쟁N" 텍스트 (N = 참여 횟수)
...
| 테이블 | 설명 |
|--------|------|
| `egm_disclosures` | 임시주주총회 공시 (393건) |
| `dispute_officers` | 분쟁 선임 임원 (391건, 255명 고유) |

### API 응답

`/api/graph-fallback/company/{id}` 응답에 `dispute_career_count` 필드 추가:
...
| 파일 | 역할 |
|------|------|
| `types/graph.ts` | `disputeCareerCount` 타입 정의 |
| `components/graph/ForceGraph.tsx` | 그래프 배지 렌더링 |
| `components/graph/NodeDetailPanel.tsx` | 상세 패널 배지 |
| `backend/app/api/endpoints/graph_fallback.py` | 배치 쿼리 |
