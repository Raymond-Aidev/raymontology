...
class BaseParser(ABC):
    ...
    def _detect_unit_from_content(self, content: str) -> int:
        ...
        v3.1 수정: 패턴 순서 변경 - 더 구체적인 패턴 먼저 (백만원 > 천원 > 원)
        ...
        patterns = [
            ...
            # 원 (마지막 - 가장 일반적인 패턴)
            (r'단\s*위\s*[:：]\s*원[^,]', 1),  # "원," 제외 (백만원, 천원 등에 포함될 수 있음)
            (r'\(단위\s*[:：]?\s*원\)', 1),
            (r'Unit\s*[:：]\s*KRW', 1),
        ]
        ...
        for pattern, multiplier in patterns:
            if re.search(pattern, content, re.IGNORECASE):
                logger.debug(f"Unit detected: {multiplier:,} (pattern: {pattern})")
                return multiplier
