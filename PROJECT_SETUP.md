...
> **마지막 업데이트**: 2026-01-23
> **상태**: 서비스 운영 중 (RaymondsIndex 포함, 조회 기록 기능 추가, 관계도 기업 노드 네비게이션 기획 완료)
...
### 2026-01-23: 관계도 기업 노드 네비게이션 기획

**기능 목적**: 관계도 그래프 내 연결된 기업 노드 클릭 시 해당 기업의 관계도로 이동하여 연쇄 리스크 분석 지원

**주요 기획 내용**:
- 더블클릭: 해당 기업 관계도로 이동 (기존: 노드 재중심)
- 싱글클릭: 상세패널 열기 (기존 유지)
- DB 기업(corp_code 존재)만 이동 가능, 비DB 기업은 시각적으로 구분
- 이용권 사전 체크 연동 (이동 전 조회 가능 여부 확인)
- NodeDetailPanel에 "이 회사 관계도 보기" 버튼 추가

**구현 상태**: 기획 완료 (코드 수정 대기)

**현재 구현된 기능 (재활용)**:
- `handleNavigateToCompany` 핸들러 (`GraphPage.tsx`)
- `pushNavigation` 히스토리 관리 (`graphStore.ts`)
- 브레드크럼 네비게이션 UI

**수정 필요 파일**:
| 파일 | 수정 내용 |
|------|----------|
| `ForceGraph.tsx` | 더블클릭 핸들러 변경, DB기업 시각적 구분 |
| `NodeDetailPanel.tsx` | "관계도 보기" 버튼 활성화 |
| `GraphPage.tsx` | 이용권 체크 연동 |

**관련 문서**: `docs/PRD_RaymondsRisk_v4.0.md` 섹션 7.4
