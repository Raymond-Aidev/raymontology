...
export default function RegisterForm({ onSuccess, onSwitchToLogin }: RegisterFormProps) {
  ...
  // 유효성 검사
  const nameError = touched.name && !name ? '이름을 입력해주세요' :
                    touched.name && name.length < 2 ? '이름은 2자 이상이어야 합니다' : ''
  ...
  const isValid = name && email && password && confirmPassword &&
                  !nameError && !emailError && !passwordError && !confirmPasswordError && agreeTerms
  ...
  }, [name, email, password, isValid, register, clearError, onSuccess])
  ...
  const getPasswordStrength = (pwd: string) => {
    ...
    if (strength <= 2) return { level: strength, text: '약함', color: 'bg-red-500' }
    if (strength <= 3) return { level: strength, text: '보통', color: 'bg-yellow-500' }
    return { level: strength, text: '강함', color: 'bg-green-500' }
  }
        ...
        <div className="relative">
          ...
          <input
            ...
            className={`w-full pl-10 pr-4 py-3 border rounded-lg transition-colors focus:outline-none focus:ring-2
                       ${nameError ? 'border-red-300 focus:ring-red-200 focus:border-red-400' :
        ...
        {password && (
          <div className="mt-2">
            ...
            <p className="text-xs text-gray-500 mt-1">
              비밀번호 강도: <span className={passwordStrength.level <= 2 ? 'text-red-600' : passwordStrength.level <= 3 ? 'text-yellow-600' : 'text-green-600'}>{passwordStrength.text}</span>
            </p>
          </div>
        )}
      </div>
